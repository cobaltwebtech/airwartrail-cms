...
export function CollectionsList({ collections = [] }: CollectionsListProps) {
	...
	const deleteCollectionMutation = useMutation(
		trpc.bunny.deleteCollection.mutationOptions({
			onSuccess: () => {
	...
	// Update collection mutation using tRPC
	const updateCollectionMutation = useMutation(
		trpc.bunny.updateCollection.mutationOptions({
			onSuccess: () => {
	...
	const handleTitleUpdate = async (title: string) => {
		if (collectionToUpdate) {
			updateCollectionMutation.mutate({
				collectionId: collectionToUpdate.guid,
				name: title,
			});
		}
	};
	...
	const handleDeleteConfirm = async () => {
		if (collectionToDelete) {
			deleteCollectionMutation.mutate({
				collectionId: collectionToDelete.guid,
			});
						...
						<CardContent className="p-4">
							<div className="flex items-start justify-between">
								<Link
									...
									params={{ collectionId: collection.guid }}
